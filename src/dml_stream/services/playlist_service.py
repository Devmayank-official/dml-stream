...
class PlaylistService:
    ...
    def get_playlist_info(self, url: str) -> Dict:
        ...
        try:
            ...
            # We use direct access with try/except because properties 
            # might trigger errors upon access
            try:
            ...
            try:
                # Try to get sidebar info safely
                sidebar = getattr(playlist, 'sidebar_info', [])
    ...
    def download_playlist(
        ...
        # Setup output folder
        playlist_name = safe_filename(title or "Playlist")
        ...
        urls_to_download = video_urls
        if max_videos:
            urls_to_download = video_urls[:max_videos]
            results['total_videos'] = len(urls_to_download)

        logger.info(f"Starting playlist download: {title}")
        logger.info(f"Total videos: {len(urls_to_download)}")
        ...
        for i, video_url in enumerate(urls_to_download):
            if self._cancel_flag:
                logger.info("Playlist download cancelled by user")
                break

            try:
                logger.info(f"Downloading video {i+1}/{len(urls_to_download)}")
