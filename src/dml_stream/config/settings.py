...
from dml_stream.core.constants import DEFAULT_CONFIG, CONFIG_SCHEMA
from dml_stream.core.exceptions import ConfigurationError
from dml_stream.utilities.platform_utils import get_config_dir, ensure_config_dir
...
class Config:
    ...
    # Download settings
    default_output_folder: str = "downloads"
    default_threads: int = 4
    max_threads: int = 12
    min_threads: int = 1
    default_method: str = "normal"
    
    # File paths (will be set relative to config dir)
    config_file_path: str = ""
    history_file_path: str = ""
    scheduled_downloads_file_path: str = ""
    batch_downloads_file_path: str = ""
    log_file_path: str = ""
    ...
    def __post_init__(self) -> None:
        """Validate configuration after initialization and set default paths."""
    ...
    def _set_default_paths(self) -> None:
        ...
        # Set file paths relative to config directory
        if not self.config_file_path:
            self.config_file_path = str(config_dir / "config.json")
        if not self.history_file_path:
            self.history_file_path = str(config_dir / "download_history.json")
        if not self.scheduled_downloads_file_path:
            self.scheduled_downloads_file_path = str(config_dir / "scheduled_downloads.json")
        if not self.batch_downloads_file_path:
            self.batch_downloads_file_path = str(config_dir / "batch_downloads.json")
        if not self.log_file_path:
            self.log_file_path = str(config_dir / "logs" / "app.log.json")
